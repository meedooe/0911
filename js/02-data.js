...
/**
 * 진행 가이드 경로 1개 (체크리스트).
 * @typedef {Object} GuidePath
 * @property {string} id      - 고유 ID
 * @property {string} title   - 경로 이름
 * @property {string} summary - 한 줄 요약
 * @property {{id:string, text:string}[]} steps - 체크할 단계들. id는 경로 내에서 중복 금지.
 */

/**
 * "막혔을 때" 진단 + 경로 안내 데이터.
 * @typedef {Object} StuckGuide
 * @property {string} statusNote - 현재 상황 요약 문구
 * @property {string[]} immediateTips - 지금 당장 써먹을 수 있는 응급 팁
 * @property {GuidePath[]} paths - 선택 가능한 진행 경로들
 */
 ...
 * @property {string[]} howItWorks - 동작 원리 설명 줄들
 * @property {{slot:string, why:string}[]} sorcTargets - 극블리 소서 기준 도박 노려볼 부위
 * @property {string} caution - 효율/기대치 관련 주의사항
 * @property {boolean} verified - 수치(확률 등)까지 검증됐는가. false면 UI에 '근사치' 표기.
 * @property {string} note - 근거/불확실성 메모
 */
...
const META_DATA = {
  ...
  /* =========================================================================
   * 6) 도박(Gheed's Gambling) 안내 — 확률/가격 수치는 패치마다 바뀌고 공식 수치가
   *    공개된 적이 없어서 넣지 않는다. 메커니즘 설명만 다룬다.
   * ========================================================================= */
  /** @type {GamblingInfo} */
  gambling: {
    host: '기드(Gheed) — 액트1 로그 야영지. 도박 창(Gamble)을 여는 대표 상인.',
    howItWorks: [
      '금화를 내고 정체불명 아이템 1개를 무작위로 받는다. 반지/목걸이/벨트/서클렛/장갑 등 아이템 종류는 직접 골라서 도박할 수 있다.',
      '가격은 아이템 종류와 내 캐릭터 레벨에 비례해서 올라간다.',
      '도박으로 나오는 아이템은 최소 매직(파란색) 등급이 보장된다. 레어·세트·유니크로 나올 가능성도 있지만 완전 무작위다.',
      '아이템 레벨은 내 캐릭터 레벨을 따라간다 — 캐릭터 레벨이 높을수록 고티어 옵션이 나올 잠재력이 커진다.',
      '식별 전에는 등급을 알 수 없다. 도박한 아이템은 반드시 직접 감정(identify)해서 확인해야 한다.'
    ],
    sorcTargets: [
      { slot: '반지', why: '요르단의 반지(SoJ), 레이븐 프로스트 등 반지 유니크가 다수라 도박 대상으로 자주 꼽힌다.' },
      { slot: '목걸이', why: '마라의 만화경 등 목걸이 유니크 풀이 넓어 도박 가치가 있다.' },
      { slot: '벨트', why: '거미 그물띠(Arachnid Mesh) 같은 벨트 유니크도 도박으로 나올 수 있다.' },
      { slot: '서클렛/투구', why: '레어 서클렛에서 +스킬/+FCR/저항 조합이 뜨면 밤날개의 너울을 못 구했을 때 대체품이 된다.' }
    ],
    caution: '유니크가 뜰 확률은 매우 낮고 공식적으로 공개된 정확한 수치가 없다. "도박으로 SoJ 파밍"은 시간 대비 효율이 낮은 보조 수단으로 보는 게 맞다 — 메인 파밍(테러존 드랍)을 우선하고 남는 골드로 여유 있게 돌리는 용도로 써라.',
    verified: false,
    note: '동작 원리(도박 시스템 자체)는 검증됐지만 정확한 확률·가격 공식은 패치별로 다르고 커뮤니티 추정치뿐이라 수치를 넣지 않았다.'
  },

  /* =========================================================================
   * 7) 진행 가이드 — "막혔을 때" 진단 + 두 갈래 경로 체크리스트
   * ------------------------------------------------------------------------
   * [TODO: LITE_MODEL_INSERT_FEATURE_HERE — 새 경로/스텝은 stuckGuide.paths 배열에 추가]
   * ========================================================================= */
  /** @type {StuckGuide} */
  stuckGuide: {
    statusNote: '81렙 소서 · 지옥(Hell) 3막 진행 중 · 박피자의 정글(Flayer Jungle)에서 독침에 막힘',
    immediateTips: [
      '해독 물약(Antidote Potion)을 벨트에 채워라 — 독 데미지와 지속시간을 즉시 줄여주는 가장 빠른 응급처치다.',
      '박피자류를 근접에서 상대하지 마라. 화면 밖 원거리에서 블리자드로 장판을 깔고 끊어내는 게 정석이다.',
      '피격 경직(히트 리커버리)이 계속 걸리면 이동/시전이 끊긴다 — 무리해서 뚫지 말고 텔레포트로 우회 동선을 만들어라.',
      '독 저항(Poison Resist)이 낮으면 체감 데미지가 확 뛴다. 장비 저항부터 점검해라.',
      '그래도 계속 죽으면 이 구간을 억지로 뚫을 이유는 없다 — 웨이포인트로 다음 지역으로 건너뛰어도 된다.'
    ],
    paths: [
      {
        id: 'gamble-path',
        title: '경로 A — 도박으로 장비 보강',
        summary: '지금 장비로 못 뚫는 거면, 냉면 없는 쉬운 구간에서 골드를 모아 반지/목걸이/벨트 도박으로 한 방을 노린다.',
        steps: [
          { id: 'gp-1', text: '치트시트 탭 "도박(Gheed\'s Gambling) 안내" 카드에서 노려볼 부위(반지/목걸이/벨트/서클렛) 확인' },
          { id: 'gp-2', text: '공포의 영역 탭에서 냉면 부담 없는 쉬운 지역(고대인의 터널 등)을 돌며 골드 겸 파밍' },
          { id: 'gp-3', text: '기드(액트1 로그 야영지)에게 가서 모은 골드로 반지/목걸이 위주 도박' },
          { id: 'gp-4', text: '나온 아이템 감정 후 저항/스킬 옵션 좋은 것으로 교체 착용' },
          { id: 'gp-5', text: '독 저항 확보한 상태로 박피자의 정글 재도전' }
        ]
      },
      {
        id: 'nightmare-path',
        title: '경로 B — 악몽에서 레벨/장비 보강',
        summary: '지옥이 아직 벅차면 악몽으로 내려가 경험치와 저항 장비를 더 채운 뒤 지옥으로 복귀한다.',
        steps: [
          { id: 'np-1', text: '악몽 난이도로 전환, 액트3 이후 구간 위주로 레벨업' },
          { id: 'np-2', text: '악몽 액트5 아냐(Anya) 구출 퀘스트 완료 — 저항 보너스 보상 확인(정확한 수치는 인게임에서 확인)' },
          { id: 'np-3', text: '마을 상인들(할브/오르무스 등)의 방어구·저항 아이템 재고 확인' },
